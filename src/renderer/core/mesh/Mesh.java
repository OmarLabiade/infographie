...
public class Mesh {
    ...
    private Vector[] computeNormals() {
        ...
        for (int i = 0; i < numFaceElements; i += VERTICES_PER_FACE) {
            ...
            int[] faces = getFaces();
            int id1 = getFaces()[i];
            int id2 = getFaces()[i+1];
            int id3 = getFaces()[i+2];
            //recuperer les sommets de notre triangle
            Vector v1 = getVertices()[id1];
            Vector v2 = getVertices()[id2];
            Vector v3 = getVertices()[id3];

            //diff

            Vector u1 = v2.subtract(v1);
            Vector u2 = v3.subtract(v1);
